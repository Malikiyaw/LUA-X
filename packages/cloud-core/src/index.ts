...
export function can(member: Member, required: Role): boolean { return roleRank[member.role] >= roleRank[required]; }
export function createProject(id: string, name: string, ownerId: string): ProjectRecord {
  ...
  const now = new Date().toISOString(); return { id, name, ownerId, members: [{ userId: ownerId, role: 'owner' }], createdAt: now, updatedAt: now };
}
...
export function recordAudit(projectId: string, actorId: string, action: string, target?: string): AuditEvent { return { id: `audit-${crypto.randomUUID()}`, projectId, actorId, action, target, at: new Date().toISOString() }; }
export function createSnapshot(projectId: string, label: string, changeIds: string[], parentId?: string): Snapshot { if (!changeIds.length) throw new Error('Snapshot must contain at least one change.'); return { id: `snap-${crypto.randomUUID()}`, projectId, label, createdAt: new Date().toISOString(), parentId, changeIds }; }
