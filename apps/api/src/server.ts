...
export async function handleApiRequest(deps: ApiDependencies, request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (method === 'GET' && (url.pathname === '/ready' || url.pathname === '/api/ready')) {
      ...
      sendJson(response, configured ? 200 : 503, { service: 'lua-x-api', ready: configured, aiProvider: configured ? 'nvidia-configured' : 'not-configured', version: API_VERSION }, { ...commonHeaders, ...rateHeaders }); return;
    }
    ...
    if (method === 'POST' && (url.pathname === '/api/ai/generate' || url.pathname === '/ai/generate')) {
      ...
      try {
        ...
        sendJson(response, 200, { requestId, provider: 'nvidia', ...(modelResult.model ? { model: modelResult.model } : { model: deps.config.nvidiaModel }), plan, rawTextAvailable: false }, { ...commonHeaders, ...rateHeaders });
      } catch (error) {
    ...
    sendJson(response, 404, { error: 'Not found.', requestId }, { ...commonHeaders, 'x-request-id': requestId });
  } catch (error) {
...
function shouldStartStandaloneServer(): boolean {
  return process.env.LUA_X_STANDALONE === 'true' && process.env.NODE_ENV !== 'test';
}

if (shouldStartStandaloneServer()) {
  const config = loadConfig();
  const nvidia = new NvidiaClient({ ...(config.nvidiaApiKey ? { apiKey: config.nvidiaApiKey } : {}), baseUrl: config.nvidiaBaseUrl, model: config.nvidiaModel, maxTokens: config.aiMaxTokens, temperature: config.aiTemperature, timeoutMs: config.aiTimeoutMs });
