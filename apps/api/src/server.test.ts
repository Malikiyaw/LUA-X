...
function deps(): ApiDependencies {
  ...
  const nvidia = new NvidiaClient({ ...config, fetchImpl: async () => new Response(JSON.stringify({ model: 'test-model', choices: [{ message: { content: JSON.stringify({ summary: 'Build a test feature', assumptions: [], changes: [], acceptanceCriteria: ['works'], verification: ['test it'], risks: [] }) } }] }), { status: 200, headers: { 'content-type': 'application/json' } }) });
...
describe('API server', () => {
  it('reports health and AI readiness', async () => withServer(async url => {
    const health = await fetch(`${url}/health`); expect(health.status).toBe(200); expect((await health.json()).status).toBe('ok');
    const ready = await fetch(`${url}/ready`); expect(ready.status).toBe(200); expect((await ready.json()).ready).toBe(true);
  }));
  ...
  it('generates a validated AI plan through the provider abstraction', async () => withServer(async url => {
    ...
    expect(body.provider).toBe('nvidia'); expect(body.plan.summary).toBe('Build a test feature');
  }));
