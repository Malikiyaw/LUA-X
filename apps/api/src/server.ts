...
export async function handleApiRequest(deps: ApiDependencies, request: IncomingMessage, response: ServerResponse): Promise<void> {
  const requestId = typeof request.headers['x-request-id'] === 'string' && request.headers['x-request-id'].length > 0 ? request.headers['x-request-id'] : randomUUID();
  ...
  try {
    ...
    if (!limit.allowed) {
      sendJson(response, 429, { error: 'Rate limit exceeded.', requestId }, { ...commonHeaders, ...rateHeaders, 'retry-after': String(Math.max(1, Math.ceil((limit.resetAt - Date.now()) / 1000))) });
      return;
    }
    ...
    if (method === 'GET' && (url.pathname === '/ready' || url.pathname === '/api/ready')) {
      ...
      sendJson(response, configured ? 200 : 503, { service: 'lua-x-api', ready: configured, aiProvider: configured ? 'nvidia-configured' : 'not-configured', version: API_VERSION }, { ...commonHeaders, ...rateHeaders });
      return;
    }
    ...
    if (method === 'POST' && (url.pathname === '/api/ai/generate' || url.pathname === '/ai/generate')) {
      if (!deps.nvidia.isConfigured()) {
        sendJson(response, 503, { error: 'AI provider is not configured on the backend.', requestId }, { ...commonHeaders, ...rateHeaders });
        return;
      }
      const payload = await readJson(request);
      if (!isGenerateRequest(payload)) {
        sendJson(response, 400, { error: 'Invalid AI request. Provide a prompt and optional project context.', requestId }, { ...commonHeaders, ...rateHeaders });
        return;
      }
      try {
        const result = await deps.nvidia.chat([
          { role: 'system', content: buildSystemPrompt() },
          { role: 'user', content: buildUserPrompt(payload) },
        ]);
        const plan = parseAIPlan(result.content);
        sendJson(response, 200, { requestId, provider: 'nvidia', model: result.model ?? deps.config.nvidiaModel, plan, rawTextAvailable: false }, { ...commonHeaders, ...rateHeaders });
      } catch (error) {
        if (error instanceof NvidiaApiError) {
          const status = error.status >= 400 && error.status < 600 ? error.status : 502;
          sendJson(response, status, { error: error.message, requestId, retryable: error.retryable }, { ...commonHeaders, ...rateHeaders });
          return;
        }
        throw error;
      }
...
/** Vercel's Node serverless entrypoint when this file is auto-detected as the function root. */
